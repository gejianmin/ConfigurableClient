...
// 公钥/私钥标签
#define kPublicKeyTag			"com.hhauto.sample.publickey"
#define kPrivateKeyTag			"com.hhauto.sample.privatekey"

//#define kRSA_Pub_Key_PEM @"MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQC5La3LfYNXLqOtn/gzO2goFKJV"\
...
#define kRSA_PUB_KEY_DER @""
...
@synthesize rsa_pub_key=_rsa_pub_key;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        [self loadPublicKey:self.rsa_pub_key];

    }
    return self;
}
-(void)setRsa_pub_key:(NSString *)rsa_pub_key{
    _rsa_pub_key=rsa_pub_key;
    [[NSUserDefaults standardUserDefaults]setObject:rsa_pub_key forKey:@"com.hhauto.pbk"];
    [self loadPublicKey:rsa_pub_key];
}
-(NSString *)rsa_pub_key{
    if (_rsa_pub_key.length==0 ) {
        NSString *keycache=[[NSUserDefaults standardUserDefaults] objectForKey:@"com.hhauto.pbk"];
        if (keycache.length==0) {
            _rsa_pub_key=kRSA_PUB_KEY_DER;
        }else{
            _rsa_pub_key=keycache;
        }
    }
    return _rsa_pub_key;
}
 ...
 *  加载公钥
 */
- (void)loadPublicKey:(NSString *)publicKey {
    
    NSAssert(publicKey.length != 0, @"公钥为空");
    
    // 删除当前公钥
    if (publicKeyRef) CFRelease(publicKeyRef);
    
    // 从一个 DER 表示的证书创建一个证书对象
    NSData *certificateData= [publicKey hh_base64DecodedData];
...
//        NSData *certificateData = [kRSAPublicKey dataUsingEncoding:NSUTF8StringEncoding];
//        
