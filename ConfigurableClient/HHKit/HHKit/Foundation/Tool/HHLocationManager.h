...
CL_EXTERN NSString *const kLocationDidUpdateNotification;
...
@property (nonatomic, strong) HHLocation *locationCache;//获取上次app定位缓存信息
/**
 *  开始定位，只定一次
...
/*! 监听定位完成，无论是GPS定位还是geo 反geo 都会做出相应的回调，可多处监听
 *  如果context相同，则会覆盖之前的监听，之前的监听则无效，要想都有相应的回调则需设置content不同即可
 */
...
@property (nonatomic, assign, readonly) CLLocationDegrees latitude;//维度
@property (nonatomic, assign, readonly) CLLocationDegrees longitude;//经度
