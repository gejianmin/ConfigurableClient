//
//  HHAPIContantString.h
//  ConfigurableClient
//
//  Created by tongda ju on 2017/8/15.
//  Copyright © 2017年 juTongDa. All rights reserved.
//

#ifndef HHAPIContantString_h
#define HHAPIContantString_h

static NSString *const kNews_HeadlineApi = @"http://61.129.118.77/storyformobile/newslist.ashx?sectionId=";/*!< newsList*/

static NSString *const kNews_HeadlineNewsId = @"http://61.129.118.77/storyformobile/newsDetail.ashx?newsId=";/*!< newsDetail*/

static NSString *const kAppPathId = @"http://apns.push0001.com/getApi.jbm?app_id=1272058382";/*!< appPathId*/

//static NSString *const kUmengAppKey = @"59991fef07fe6560800019f5";/*!<UmengAppKey*/
static NSString *const kUmengAppKey = @"599e40e2f29d987c6d0007df";/*!<UmengAppKey*/


#endif /* HHAPIContantString_h */
